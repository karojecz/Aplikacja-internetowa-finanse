<?php
	...
	if ($polaczenie->connect_errno!=0)
	{
		...
		$expense_sum[$i]=$wiersz1['sum'];
		$expense_name[$i]=$wiersz1['expense_name'];
		$sum_all_expenses+=$wiersz1['sum'];
		}
		
		//pobranie danych o przychodach i kategoriach z obecnego miesiaca
		$_SESSION['incomes']=$polaczenie->query("SELECT SUM(incomes.amount) AS sum, incomes_category_assigned_to_users.name AS income_name FROM incomes INNER JOIN incomes_category_assigned_to_users ON(incomes.income_category_assigned_to_user_id=incomes_category_assigned_to_users.id AND incomes.user_id=$user_id AND incomes.date_of_income>='$firstday' AND incomes.date_of_income<='$lastday') GROUP BY incomes_category_assigned_to_users.name ORDER BY incomes.amount;");
		...
		$income_sum[$i]=$wiersz1['sum'];
		$income_name[$i]=$wiersz1['income_name'];
...
var dataArray = [['Year','Total Average']];
				...
				var expense_names =<?php echo json_encode($expense_name); ?>;
				var expense_sums =<?php echo json_encode($expense_sum); ?>;
				var datki=<?php echo $firstday; ?>
		...
		<nav class="navbar navbar-dark bg-jumpers navbar-expand-lg">
			...
			<div class="collapse navbar-collapse" id="mainmenu">
				...
				<div class="navbar-nav mr-auto">
						...
						<h2> <?php echo "$firstday - $lastday" ?> </h2>
			...
			<div class="row">
					<div class="col-md-4 p-2">
						<table class="table table-bordered  ">
						   ...
						   echo
							  '<tr>
								 <th>'.$expense_sum[$i].'</th><td>'.$expense_name[$i].'</td> 
							  </tr>';
					...
					<div class="col-md-4 p-2">
						...
						<?php
						   ...
						   echo
							  '<tr>
								 <th>'.$income_sum[$i].'</th><td>'.$income_name[$i].'</td> 
							  </tr>';
						   }
